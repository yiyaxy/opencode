...
| Bun 类型检查、构建和测试 | 部分完成 | Bun 已安装；13 个基础包类型检查、App/服务端构建和当前平台 CLI/单体构建已通过；未新增或运行测试 |
...
1. 保留当前 `internal-cleanup` 作为备份，确认公开 Fork 的分支与权限设置。
2. 重建基于 `upstream/dev` 的正确 Git 历史并迁移当前清理变更。
3. 暂停直接删除 `web`、`console`、`slack` 等可能被 B/C 复用的模块。
4. 产出模块“删除、隔离、保留、未来复用”审计清单。
5. 安装并锁定 Bun，补齐当前清理结果的类型检查、既有测试和构建验证。
6. 产出公共域名、模型、遥测、更新和凭据使用清单。
7. 进入 M1，先交付完整、可靠、安全的内部编程助手闭环。

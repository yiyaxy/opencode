...
- 建设可恢复的长文转化提取流水线，支持最多 8 万汉字的分块处理、断点续跑和来源追溯，详见 [八万字长文转化提取计划](./long-content-extraction-plan.md)。
- 建立内部安装包、签名、升级和回滚流程。
...
- 当前已在模型设置页增加“设为默认/默认”操作，覆盖旧版和新版设置界面；功能分支 `model-sharing` 已推送到 Fork，待 PR #1 合并到 `dev` 后再基于最新 `dev` 提交合并请求。
- 已移除运行时 Agent 提示中强制访问官方文档和提交公共反馈的引导；内部产品后续通过本地文档和公司反馈渠道承接。
- 已统一构建期模型快照与运行时模型目录的默认地址，并保留 `OPENCODE_MODELS_URL` 与 `MODELS_DEV_API_JSON` 覆盖入口；公司模型网关地址确认后可沿用同一切换路径。
...
| A 层内部化 | 进行中 | 已实现服务器默认模型设置、移除运行时公共文档引导并统一模型目录默认值；外部端点、品牌、模型网关、安装更新待处理 |
| 长文转化提取 | 进行中 | 30 分钟常规演讲为主场景，8 万字为极限测试；已落地交互/批处理自动双档切块、来源区间、失败动作分类和任务/分块/结果持久化骨架，Provider 调度、重试、性能验收和分层合并待实现 |
...
8. 按 [八万字长文转化提取计划](./long-content-extraction-plan.md) 先实现核心模板，再集中完成持久化恢复、产品界面和故障测试。
